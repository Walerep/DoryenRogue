...
// consts with colors and light radiuses
...
pub struct Level {
    /// a picture containing color coded walls, 
    /// player start pos and entities. 
    /// subcell resolution (2x2 pixel for each console cell)
    level_img: Image,
    /// this level ground texture, subcell resolution
    ground: Image,
    /// if level img has been loaded
    loaded: bool,
    /// map of lightsin the level
    lightmap: Image,
    /// level size
    size: (i32, i32),
    /// level start position
    start: (i32, i32),
    /// if cell have collision or not
    walls: Vec<bool>,
    /// what part of level has been visited
    visited_2x: Vec<bool>,
    /// utility to compute FOV
    fov: FovRestrictive,
    /// what part of level in players FOV
    map: MapData,
    /// the final background image
    render_output: Image,
    /// dynamic light in level
    lights: Vec<Light>,
    /// light of player
    player_light: Light,
}
...
impl Level {
    ...
    pub fn compute_fov(&mut self, (x,y): (i32, i32), radius: usize) {
        self.map.clear_fov();
        self.fov.compute_fov(&mut self.map, x as usize * 2, y as usize * 2, radius, true);
    }
    ...
    fn compute_lightmap(&mut self, (px, py): (i32, i32)) {
        ...
        *self.player_light.pos_mut() = ((px * 2) as f32, (py * 2) as f32);
        self.player_light.render(&mut self.map, &mut fov, &mut self.lightmap, false);
        for light in self.lights.iter() {
            light.render(&mut self.map, &mut fov, &mut self.lightmap, false);
        }
    }
    fn compute_walls_2x_and_start_pos(&mut self) -> Vec<Entity> {
        ...
        for y in 0..image_size.1 {
            for x in 0..image_size.0 {
                ...
                self.map.set_transparent(x as usize, y as usize, p != WALL_COLOR);
